...
class MTTFMetricTest {
    ...
    @Test
    void should_0_seconds_when_1_failed_and_1_success_builds() {
        List<BuildMessage> builds = Lists.newArrayList(THIRD_BUILD, FOURTH_BUILD);
        runAndVerifyResult(builds, 0, 0);
    }

    @Test
    void should_correct_metric_when_1_success_and_1_failed_builds() {
        List<BuildMessage> builds = Lists.newArrayList(FIRST_BUILD, SECOND_BUILD);
        runAndVerifyResult(builds, 1000L, 1);
    }

    @Test
    void should_correct_metric_when_2_success_and_1_failed_builds() {
    ...
    @Test
    void should_return_failed_info_when_starting_with_a_failed_build() {
        List<BuildMessage> builds = Lists.newArrayList(SECOND_BUILD, THIRD_BUILD,
                FOURTH_BUILD, FIFTH_BUILD, SIXTH_BUILD);
        runAndVerifyResult(builds, 1000L, 1);
    }
    private void runAndVerifyResult(List<BuildMessage> builds, long expectTime, int expectCount) {
        ...
        assertEquals("test", mttfMetric.getName(), "Metric Name");
        assertEquals(expectTime, mttfMetric.calculateMetric(), "MTTF Metric");
        assertEquals(expectCount, mttfMetric.getOccurences(), "Build Count");
    }
}
